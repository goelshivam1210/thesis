\begin{figure}[t]
    ...
    % MDP states
    \node[envstate] (e0) at (-3.2, -3.45) {$s_0$};
    \node[envstate] (e1) at (-0.2, -3.45) {$s_1$};
    \node[envstate] (e2) at (2.8, -3.45) {$s_2$};

    % Executors (positioned on the arrows)
    \draw[exflow] (e0) -- (e1) node[midway, below=3pt, exnode] (x1) {$e(\Operator_1)$};
    \draw[exflow] (e1) -- (e2) node[midway, below=3pt, exnode] (x2) {$e(\Operator_2)$};
    ...
    \end{tikzpicture}
    \caption[Agent--environment interaction in the hybrid framework]{The two-level structure of the hybrid planning and learning framework. \textbf{Top (symbolic):} The agent generates a plan $\Plan$ as a sequence of operators $\Operator_i$ transitioning between symbolic states $\SymState_i \in \SymStates$. \textbf{Bottom (subsymbolic):} Execution occurs in MDP states $s_i \in \States$, and transitions are realized by executors $e(\Operator_i)$---policies that implement the corresponding operators. The abstraction function $\Abstraction : \States \to \SymStates$ maps MDP states to symbolic descriptions. Each operator--executor pair (dashed orange) links a symbolic action to its realization in the MDP. Novelty manifests when $e(\Operator_i)$ no longer achieves the intended symbolic transition despite a valid plan.}
