\begin{figure}[htbp]
    ...
    \end{axis}
    \end{tikzpicture}
    \caption{Bar-chart visualization of the adaptation time ($T_{\text{adapt}}$, lower is better, log scale) reported in \cref{tab:rapid_learn_results}. Error bars denote one standard deviation over 10 independent trials. All RAPid-Learn variants (KGE-UCB, KGE-UAB, EG) adapt one to two orders of magnitude faster than the transfer learning baselines. Transfer learning baselines did not converge on the RT hard scenario and are omitted from that group.}
