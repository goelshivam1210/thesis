\begin{figure}[htbp]
    ...
    \end{axis}
    \end{tikzpicture}
    \caption[\FLEX{} success rates on novel objects]{Bar-chart visualization of the \FLEX{} success rates reported in \cref{tab:flex_results}, evaluated on novel object instances with the Panda robot. Error bars denote one standard deviation. Cabinets and trashcans are prismatic joints; microwaves and dishwashers are revolute joints. \FLEX{} achieves the highest success rate on every object type while requiring substantially less training than all reinforcement learning baselines (end-to-end RL fails to exceed a $1\%$ success rate within $\geq$10~M time steps).}
