...
The algorithm first calls the planner once, which costs $C_{\text{plan}}$.
It then constructs the initial executor set by mapping each operator in the plan
to its executor, which costs $O(|\Plan|)$. The main loop executes each operator
once, giving $O(|\Plan| \cdot C_{\text{exec}})$ execution cost. For each failed
operator, the algorithm either executes an already discovered executor or calls
\textsc{Discover-Executor} and then executes the returned executor. Therefore,
the total cost is
...
|\Plan| C_{\text{exec}}
+
F(C_{\text{disc}} + C_{\text{exec}})
\right).
\]

Since $F \leq |\Plan|$, the worst-case time complexity is $O\!\left(
C_{\text{plan}}
+
|\Plan|
+
|\Plan| C_{\text{exec}}
+
|\Plan|(C_{\text{disc}} + C_{\text{exec}})
\right)$,
which simplifies to $O\!\left(
C_{\text{plan}} + |\Plan|(C_{\text{disc}} + C_{\text{exec}}) \right)$.


The space required by the top-level algorithm is dominated by the plan and the
executor set. Thus, excluding the internal memory used by the planner and
...
\begin{itemize}
  ...
  bounded-arity domains, this reduces to $O(|\mathcal{P}|)$.
  \item \textbf{Space:} $O(|\mathcal{P}|\psi_{\max}+\omega_{\max})$, reducing
  to $O(|\mathcal{P}|)$ in bounded-arity domains.
\end{itemize}
A \emph{bounded-arity domain} is one in which every predicate and operator takes
at most a fixed constant number of arguments (as in standard planning
benchmarks), so $\psi_{\max}$ and $\omega_{\max}$ are constants independent of
the plan length and the number of objects in the problem.}
