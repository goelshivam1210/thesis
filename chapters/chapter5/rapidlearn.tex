...
\begin{figure}
    ...
    \caption{Overview of the \RAPidLearn{} framework.}
...
\begin{figure}[t]
    ...
    \caption[\RAPidLearn{} recovery from the rubber-tree novelty]{Overview of the agent's response to novelty. (a) The agent operates in the pre-novelty \NovelGridworlds{} domain. (b) The environment introduces the \emph{rubber-tree-hard} novelty, causing an impasse. (c) During executor discovery, the agent accommodates the novelty using RAPid-KGE-UCB. (d) The agent resolves the plan-execution impasse. (e) The agent reaches the goal state by crafting a \emph{pogo-stick}.}
