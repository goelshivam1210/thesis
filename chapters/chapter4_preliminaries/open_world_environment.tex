...
This gap between the agent's knowledge and the true state of the world is not merely a quantitative shortcoming that can be addressed by training on more data or enumerating more states. It is a qualitative property of the agent--environment relationship: the world may contain structure that is not derivable from, or even expressible within, the agent's current representational framework. We developed this gap conceptually in \cref{ch:novelty_open_world}; the remainder of this chapter gives it a formal vocabulary.

\subsection{A Running Example: The Crafting Agent}
\label{sec:running_example}
\label{sec:formal_running_example}

We use a simple crafting domain as a running example throughout the dissertation. The agent operates in a gridworld environment inspired by Minecraft-like crafting tasks (\cref{fig:gridworld}). The environment contains objects such as trees, crafting tables, ores, and storage containers. The agent's goal is to craft a target item, such as a pogostick, by collecting resources and combining them according to known recipes.

The agent represents this task at two levels. At the symbolic level, it knows facts about objects and recipes and uses operators such as \texttt{break\_tree}, \texttt{craft\_planks}, and \texttt{craft\_pogostick}. At the execution level, each operator is carried out by an \emph{executor}: a policy or procedure that navigates to a tree, interacts with it, and so on. Under closed-world conditions, where the environment matches the agent's models, generating a plan and executing it operator by operator succeeds reliably.

Now suppose the environment changes while the agent is performing its task. A new entity---a supplier---appears and may affect the agent's ability to obtain resources. Or the properties of a known object change: trees become unbreakable unless the agent first acquires a new tool, an axe. Or a crafting recipe is altered so that a previously unnecessary ingredient is now required. Under the agent's original model, executing \texttt{break\_tree} near a tree produces logs; if trees now require an axe, the agent may still generate a plan containing \texttt{break\_tree}, but the corresponding executor no longer achieves the expected effect. Each of these cases is a candidate novelty when the relevant change is not derivable from the agent's current knowledge. The agent framework and novelty formalism developed next provide the machinery for analyzing these situations precisely.
