...
\vspace*{0.2cm}
\footnotesize
\caption{\footnotesize Reactive Force Estimation via Impedance Probing}
...
a sample and evicting the oldest are $O(1)$ operations. The
algorithm performs at most $N$ probing iterations. In each iteration, it updates
the force estimate, maintains a sliding window of at most $w$ samples, and
computes the window mean. If the mean is computed directly, each iteration costs
$O(w)$, giving a total time complexity of $O(Nw)$. If the sliding-window mean is maintained incrementally using a running sum, then
each iteration takes constant time, and the total time complexity reduces to $O(N)$. The space complexity is
$O(w)$ since the algorithm stores only the most recent $w$ force samples.}
