\section{Force Policies for Rigid-Body Transport}
\label{sec:pho}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/chapter6/arch.pdf}
    \caption[Architecture of the pushing framework]{Architecture of the pushing heavy objects framework. The policy is trained in simulation (top right) using a parameterized MDP conditioned on an interaction descriptor $z$. At deployment (bottom), the robot estimates $z$ through impedance-based force probing and constructs the object-centric state from proprioceptive sensing, then executes the learned boundary forces through compliant impedance control.}
...
\vspace*{0.2cm}
\footnotesize
\caption{Reactive force estimation via impedance probing.}
\label{alg:force_probe}
\begin{algorithmic}[1]
\REQUIRE Probe axis $\hat{u}$, step size $\delta$, max steps $N$, window size $w$, break threshold $\Delta_f$, $F_{\max}$
\ENSURE $z \in [0,1]$
\STATE Activate impedance controller and wait for settling
\STATE $f_{\text{base}} \gets \mathrm{mean}(\text{baseline samples})$
\STATE $f_{\max} \gets 0$
\STATE $\mathcal{W} \gets \varnothing$
\FOR{$i = 1$ to $N$}
    \STATE Advance virtual reference by $i\delta$ along $\hat{u}$
    \STATE $f_i \gets \max_t (f_{\text{proj}}(t) - f_{\text{base}})$
    \STATE $f_{\max} \gets \max(f_{\max}, f_i)$
    \STATE Append $f_i$ to $\mathcal{W}$ (retain last $w$ samples)
    \IF{$|\mathcal{W}| = w$}
        \STATE $\bar{f} \gets \mathrm{mean}(\mathcal{W})$
        \IF{$f_{\max} - \bar{f} > \Delta_f$}
            \STATE \textbf{break} \COMMENT{breakaway detected}
        \ENDIF
    \ENDIF
\ENDFOR
\STATE \RETURN $z \gets \mathrm{clip}(f_{\max}/F_{\max}, 0, 1)$
\end{algorithmic}
\end{algorithm}
...
Let $N$ be the maximum number of probing steps and $w$ be the window size.
Assume the sliding window is maintained in a fixed-size buffer so that appending
a sample and evicting the oldest are $O(1)$ operations. With an incremental
running sum for the sliding-window mean, each probing iteration takes constant
time, so the total time complexity is $O(N)$. The space complexity is $O(w)$
because the algorithm stores only the most recent $w$ force samples. If the
window mean is recomputed from scratch at each step, the time complexity becomes
$O(Nw)$. This analysis assumes that each probing step provides a summarized
force estimate $f_i$ or uses a fixed-size low-level sensing window.}
...
\input{tables/chapter6/pho_ood}
\input{figures/chapter6/pho_ood_bars}
...
\input{tables/chapter6/pho_paths}
\input{figures/chapter6/pho_paths_bars}
...
\input{tables/chapter6/pho_ablation}
...
\begin{figure}[t]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter6/8pt8kg-policy.jpg}
        \caption{Policy, 8.8\,kg}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter6/8pt8kg-baseline.jpg}
        \caption{Baseline, 8.8\,kg}
    \end{subfigure}
    \\[6pt]
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter6/12pt5kg-Policy.jpg}
        \caption{Policy, 12.5\,kg}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter6/12pt5kg-baseline.jpg}
        \caption{Baseline, 12.5\,kg}
    \end{subfigure}
    \caption[Real-world box transport with the Boston Dynamics Spot]{Chronophotographic composites of real-world box transport on the Boston Dynamics Spot. Top row: proposed force-based policy under $8.8$~kg (a) and $12.5$~kg (c) payloads, showing consistent trajectory tracking. Bottom row: kinematic baseline under the same conditions (b, d), showing frequent deviation and failure to reach the goal.}
