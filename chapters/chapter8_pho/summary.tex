...
Together, \cref{ch:flex,ch:pho} provide the generalization counterpart to the accommodation we examined in \cref{ch:rapid_learn}. The results in Chapter~\ref{ch:rapid_learn} show how action abstractions localize the behavior that must be learned after an execution failure. The results in Chapters~\ref{ch:flex} and~\ref{ch:pho} show how interaction abstractions allow a fixed policy to remain applicable across the evaluated variation in objects, physical conditions, and embodiments. The contributions therefore support the two sides of the dissertation's technical argument: structured abstractions can localize revision when existing competence is insufficient and can reduce the need for revision by capturing invariances that support generalization.
