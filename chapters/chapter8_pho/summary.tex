...
This chapter presented an object-centric force-policy framework for pushing heavy rigid objects along prescribed paths under unknown mass and friction. The policy is trained in simulation using force-space actions and an interaction descriptor that summarizes contact resistance. At deployment, the robot estimates this descriptor through lightweight probing and executes the learned force commands through compliant control. Experiments on a Boston Dynamics Spot mobile manipulator demonstrate zero-shot sim-to-real transfer for heavy-object transport.

The framework extends the interaction abstraction perspective from \cref{ch:flex}. FLEX showed how force-space policies can generalize across articulated objects and robot arms; this chapter showed that the same abstraction principle applies to rigid body transport across physical parameters and mobile manipulation settings. Together, \cref{ch:flex,ch:pho} demonstrate that object-centric force-space representations can preempt some forms of mismatch by making policies invariant to robot-specific control details and robust to task-relevant physical variation.
