...
The benchmark task in NovelGym is a Minecraft-inspired pogostick crafting task. The agent operates in a two-room gridworld environment containing resources (oak trees, diamond ore, platinum blocks), a crafting table, and a trader. The agent must collect resources, craft intermediary items, trade with the trader, and ultimately craft a pogostick. A simplified version of the task was used to make it tractable for reinforcement learning agents while preserving the sequential, multi-step nature of the crafting challenge.

% FIGURE PLACEHOLDER: NovelGym environment representation (Figure 1 from the paper)
\begin{figure}[t]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/chapter3/Picture1_with_novelty.png}
    \caption{NovelGym environment representation showing a gridworld with various entities. The red box highlights a novelty (axe) introduced in the environment.}
...
All agents achieved perfect pre-novelty performance ($S_\text{pre} = 1.0$), with the RL agents requiring 4 million training time steps to reach this level. Results across five novelties and four agents are summarized below; see~\citet{goel2024novelgym} for the full quantitative tables\footnote{All results are averaged across 10 random seeds. A convergence criterion based on success rate and reward thresholds was used to determine when adaptation was complete; see~\citet{goel2024novelgym} for the precise convergence criteria and hyperparameter settings.}.

% TABLE PLACEHOLDER: Full results table (Table 2 from the paper)
\begin{table}[t]
\footnotesize
\centering
...
\bottomrule
\end{tabular}
\caption{Performance metrics across novelty scenarios.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Efficiency metrics across novelty scenarios.}
\end{table}
...
% \toprule
% Novelty & Agent & {$S_{\text{pre}} \uparrow$} & {$I_{\text{nov}} \downarrow$} & {$T_{\text{adapt}} \downarrow$} & {$S_{\text{post}} \uparrow$} & {$\Delta t \downarrow$} \\
% \midrule

% \textbf{Axe} & RapidLearn$^{+}$(PPO+ICM) & 1 +- 0 & 0.83 +- 0.205 & 71040 +- 17413 & 1.0 +- 0.0 & 122.7 +- 15.61 \\
%              & RapidLearn (PPO)          & 1 +- 0 & 0.69 +- 0.207 & 68160 +- 17934 & 1.0 +- 0.0 & 116.9 +- 9.88 \\
%              & Transfer RL (PPO)+ICM     & 1 +- 0 & 1.0 +- 0.0   & 169440 +- 97949 & 0.97 +- 0.021 & 61.0 +- 8.95 \\
%              & Transfer RL (PPO)         & 1 +- 0 & 1.0 +- 0.0   & 114240 +- 19651 & 0.97 +- 0.018 & 63.1 +- 8.17 \\
% \midrule

% \textbf{Chest} & RapidLearn$^{+}$(PPO+ICM) & 1 +- 0 & {--} & {--} & {--} & {--} \\
%                & RapidLearn (PPO)          & 1 +- 0 & {--} & {--} & {--} & {--} \\
%                & Transfer RL (PPO)+ICM     & 1 +- 0 & 0.0 +- 0.004 & 24000 +- 0 & 1.0 +- 0.0 & -3.0 +- 0.78 \\
%                & Transfer RL (PPO)         & 1 +- 0 & 0.01 +- 0.009 & 24000 +- 0 & 0.98 +- 0.011 & -3.5 +- 3.13 \\
% \midrule

% \textbf{Trader} & RapidLearn$^{+}$(PPO+ICM) & 1 +- 0 & 0.45 +- 0.069 & 102720 +- 20724 & 0.95 +- 0.017 & 126.9 +- 9.97 \\
%                 & RapidLearn (PPO)          & 1 +- 0 & 0.5 +- 0.07   & 96480 +- 21191 & 0.96 +- 0.02  & 122.8 +- 8.93 \\
%                 & Transfer RL (PPO)+ICM     & 1 +- 0 & 0.96 +- 0.108 & 227040 +- 112120 & 0.99 +- 0.011 & 93.6 +- 12.05 \\
%                 & Transfer RL (PPO)         & 1 +- 0 & 0.94 +- 0.128 & 350880 +- 237039 & 0.96 +- 0.025 & 89.3 +- 7.87 \\
% \midrule

% \textbf{Fence} & RapidLearn$^{+}$(PPO+ICM) & 1 +- 0 & 0.38 +- 0.102 & 69120 +- 15657 & 0.96 +- 0.02 & 161.6 +- 7.6 \\
%                & RapidLearn (PPO)          & 1 +- 0 & 0.41 +- 0.052 & 66400 +- 20999 & 0.95 +- 0.008 & 168.7 +- 6.15 \\
%                & Transfer RL (PPO)+ICM     & 1 +- 0 & 1.0 +- 0.0   & 854400 +- 52974 & 0.93 +- 0.02 & 73.4 +- 4.7 \\
%                & Transfer RL (PPO)         & 1 +- 0 & 1.0 +- 0.0   & 701760 +- 100589 & 0.92 +- 0.024 & 90.9 +- 9.49 \\
% \midrule

% \textbf{Fire} & RapidLearn$^{+}$(PPO+ICM) & 1 +- 0 & 1.0 +- 0.0   & 263520 +- 81979 & 0.95 +- 0.02 & 133.2 +- 21.38 \\
%               & RapidLearn (PPO)          & 1 +- 0 & 1.0 +- 0.0   & 252000 +- 49663 & 0.95 +- 0.016 & 142.5 +- 13.71 \\
%               & Transfer RL (PPO)+ICM     & 1 +- 0 & 1.0 +- 0.0   & 372960 +- 198096 & 0.94 +- 0.016 & 105.3 +- 9.85 \\
%               & Transfer RL (PPO)         & 1 +- 0 & 1.0 +- 0.0   & 127200 +- 37932 & 0.96 +- 0.029 & 103.3 +- 13.22 \\

% \bottomrule
% \end{tabularx}
% \caption{Evaluations of the agents across five novelty scenarios. $\uparrow$ indicates higher is better, $\downarrow$ lower is better. -- indicates not applicable.}
% \label{tab:results}
% \end{table}

\paragraph{Novelty impact.} Upon novelty injection, all agents experienced performance degradation for detrimental novelties, with the fire novelty producing the most severe impact ($I_\text{novelty} = 1.0$ for all agents). Transfer RL agents were generally more sensitive to novelty than hybrid agents, as reflected in higher $I_\text{novelty}$ values. However, hybrid agents showed lower immediate impact in some cases due to their targeted adaptation approach, which activates only upon execution failure.

\paragraph{Adaptation speed.} Hybrid agents consistently outperformed transfer learning methods on the $T_\text{adapt}$ metric across all novelties. For the fence novelty, hybrid agents adapted in approximately 67{,}000--71{,}000 time steps, while transfer RL agents required 700{,}000--854{,}000 time steps. This advantage arises from the hybrid architecture's ability to focus learning on the specific subproblem caused by the novelty rather than relearning the entire task.

\paragraph{Post-adaptation performance.} All agents achieved high post-adaptation success rates ($S_\text{post} \geq 0.92$), though none reached perfect scores due to the use of a convergence criterion rather than exhaustive training.

\paragraph{Beneficial novelty.} The chest novelty was not detected as a novelty by hybrid agents, since it did not cause execution failure. Transfer RL agents, however, discovered and exploited the chest through exploration, as reflected in negative $\Delta t$ values indicating improved task completion efficiency.

\paragraph{Role of curiosity-driven exploration.} The addition of ICM improved adaptation in some cases but was not uniformly beneficial. In the fire novelty scenario, ICM slightly degraded the performance of the transfer RL agent, suggesting that intrinsic curiosity can sometimes interfere with goal-directed adaptation.