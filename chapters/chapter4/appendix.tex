...
\label{sec:benchmarks_technical_appendix}
...
\rev{The hybrid agent's action space contains higher-level operators, such as \texttt{approach <entity>}, together with the primitive actions used by their executors. For the RL agent, every higher-level operator is grounded to a specific argument. The complete grounded action set used in the experiments is listed below.}
...
\begin{tabularx}{0.92\textwidth}{>{\ttfamily\raggedright\arraybackslash}X>{\ttfamily\raggedright\arraybackslash}X}
\toprule
\multicolumn{2}{c}{\normalfont\textbf{Grounded action set}} \\
\midrule
\rowcolor{codeBackground}
collect & break\_block \\
approach\_oak\_log & approach\_diamond\_ore \\
\rowcolor{codeBackground}
approach\_crafting\_table & approach\_block\_of\_platinum \\
approach\_entity\_103 & interact\_103 \\
\rowcolor{codeBackground}
select\_oak\_log & select\_iron\_pickaxe \\
select\_sapling & select\_tree\_tap \\
\rowcolor{codeBackground}
select\_crafting\_table & deselect\_item \\
craft\_stick & craft\_planks \\
\rowcolor{codeBackground}
craft\_block\_of\_diamond & craft\_pogo\_stick \\
trade\_block\_of\_titanium\_1 & move\_forward \\
\rowcolor{codeBackground}
move\_backward & move\_left \\
move\_right & rotate\_left \\
\rowcolor{codeBackground}
rotate\_right & place \\
<slot 1> & <slot 2> \\
\bottomrule
...
\rev{The two placeholder actions, \texttt{<slot 1>} and \texttt{<slot 2>}, are assigned as needed in post-novelty settings.}
...
\input{chapters/chapter4/novelties_table}
...
\rev{The reward-shaping mechanism works as follows. The planner generates a plan $P$ for the current task, from which an ordered queue $\mathcal{Q}$ of subgoals is extracted. For the pogostick task, the queue contains the following sequence.}
...
\arrayrulecolor{codeRule}
\begin{tabularx}{0.80\textwidth}{>{\centering\arraybackslash}p{0.10\textwidth}>{\raggedright\arraybackslash}X}
\toprule
\textbf{Step} & \textbf{Planner-derived subgoal} \\
\midrule
\rowcolor{codeBackground}
1 & \texttt{break \textlangle block of platinum\textrangle} \\
2 & \texttt{trade block of titanium} \\
\rowcolor{codeBackground}
3 & \texttt{break block of diamond} \\
4 & \texttt{break tree} \\
\rowcolor{codeBackground}
5 & \texttt{craft planks} \\
6 & \texttt{craft stick} \\
\rowcolor{codeBackground}
7 & \texttt{craft pogostick} \\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{lstlisting}[
  style=yamlstyle,
  caption={NovelGym configuration for a simple resource-collection and crafting task.},
  label={lst:ng-sample-config}
]
---
actions:
  break_block:
...
\input{chapters/chapter4/simplified_domain}

\subsubsection{Pre-Novelty Problem}

\rev{The pre-novelty problem is defined as follows:}
\input{chapters/chapter4/simplified_problem}
...
\begin{lstlisting}[
  style=yamlstyle,
  caption={YAML configuration for the busy-trader novelty.},
  label={lst:ng-busy-trader-config}
]
---
novelties:
  '0':
...
\begin{lstlisting}[
  style=pythonstyle,
  caption={Python implementation of the busy-trader action.},
  label={lst:ng-busy-trader-python}
]
