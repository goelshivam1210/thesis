...
\NovelGym{}\footnote{The project website and codebase are available at \url{https://github.com/tufts-ai-robotics-group/NovelGym}.} extends and generalizes the ideas introduced in \NovelGridworlds{} into a flexible ecosystem for developing and benchmarking open-world agents. While \NovelGridworlds{} provides a fixed set of tasks and novelties suitable for initial evaluation, \NovelGym{} introduces a formal framework for defining environments and their transformations, a modular software architecture for rapid development of tasks, novelties, and agent systems, and a comprehensive evaluation protocol with adaptation-focused metrics. The ecosystem supports single-agent and multi-agent scenarios, accommodates symbolic planning agents, reinforcement learning agents, and hybrid architectures, and enables composable novelty injection. The environment representation introduced in \cref{sec:ng_environment} serves as the basis for the formalism presented below.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/chapter4/sensor_rep.png}
    \caption[Sensor representations in \NovelGym{}]{Illustration of the agent's sensor representations in \NovelGym{}. (Left) A LiDAR-like representation emitting beams at angular increments. (Right) An image-based local view representation using one-hot vector encodings.}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/chapter4/novelgym_architecture.pdf}
    \caption[System design of the \NovelGym{} ecosystem]{System design of the \NovelGym{} ecosystem. Blue highlights the environment modules and purple highlights the agent modules. The novelty injector (red) transforms the environment based on specified configurations.}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        >=Stealth,
        axisline/.style={->, thick, gray!70!black},
        curve/.style={very thick, black},
        baseline/.style={thick, blue!65!black},
        inject/.style={very thick, red!80!black},
        measure/.style={<->, thick},
        phaselabel/.style={font=\small\bfseries, gray!35!black},
        annot/.style={font=\footnotesize},
    ]
        % ----- geometry -----
        % x: time (0..12), y: success rate (0..4.4)
        \def\xmax{12.2}
        \def\ymax{4.6}
        \def\xinj{5.6}     % novelty injection time
        \def\ybase{0.9}    % random baseline success rate
        \def\ypre{3.9}     % pre-novelty asymptote
        \def\ypost{3.7}    % post-novelty asymptote
        \def\ximm{6.05}    % x-position of the novelty-impact annotation
        \def\xconv{9.9}    % post-novelty convergence time

        % ----- axes -----
        \draw[axisline] (0,0) -- (\xmax,0) node[below left=1pt and 0pt, font=\small] {time};
        \draw[axisline] (0,0) -- (0,\ymax) node[above right=0pt and -2pt, font=\small, align=left, rotate=90, anchor=south west] {};
        \node[font=\small, rotate=90] at (-0.55, {\ymax/2}) {success rate};

        % ----- random baseline -----
        \draw[baseline] (0,\ybase) -- (\xmax,\ybase);
        \node[annot, blue!65!black, anchor=west] at (\xmax, \ybase) {};
        \node[annot, blue!65!black, anchor=east, fill=white, inner sep=1pt] at (\xmax, {\ybase+0.32}) {random baseline};

        % ----- novelty injection line -----
        \draw[inject] (\xinj,0) -- (\xinj,\ymax);
        \node[annot, red!80!black, anchor=south, align=center] at (\xinj, \ymax) {novelty\\[-2pt]injection};

        % ----- phase labels -----
        \node[phaselabel] at ({\xinj/2}, {\ymax-0.35}) {pre-novelty};
        \node[phaselabel] at ({(\xinj+\xmax)/2}, {\ymax-0.35}) {post-novelty};

        % ----- pre-novelty learning curve (starts AT the random baseline) -----
        \draw[curve] plot[smooth, tension=0.7] coordinates {
            (0,\ybase) (0.8,1.0) (1.6,1.5) (2.4,2.4) (3.3,3.3) (4.2,3.75) (5.0,3.88) (\xinj,\ypre)
        };

        % ----- post-novelty learning curve (restarts FROM the baseline point;
        %        no falling line connecting it to the pre-novelty peak) -----
        \draw[curve] plot[smooth, tension=0.7] coordinates {
            (\xinj,\ybase) (6.6,1.15) (7.4,1.95) (8.3,2.9) (9.2,3.5) (\xconv,\ypost) (11.4,3.72)
        };

        % ----- pre-novelty asymptotic performance -----
        \draw[measure, brown!70!black] (4.55,0) -- (4.55,\ypre);
        \node[annot, brown!70!black, anchor=west] at (2.05,{\ypre/2+0.15}) {$S_{\text{pre}}$ (asymptotic perf.)};

        % ----- novelty impact (drop from pre-novelty asymptote to baseline) -----
        \draw[measure, orange!85!black] (\ximm,\ybase) -- (\ximm,\ypre);
        \node[annot, orange!85!black, anchor=west] at ({\ximm+0.1},{(\ybase+\ypre)/2}) {$I_{\text{novelty}}$};

        % ----- time to adapt -----
        \draw[measure, green!45!black] (\xinj,-0.45) -- (\xconv,-0.45);
        \node[annot, green!45!black, anchor=north] at ({(\xinj+\xconv)/2},-0.5) {$T_{\text{adapt}}$ (time to adapt)};

        % ----- post-novelty asymptotic performance -----
        \draw[measure, brown!70!black] (10.9,0) -- (10.9,\ypost);
        \node[annot, brown!70!black, anchor=south, align=center] at (10.9,\ypost) {$S_{\text{post}}$};
    \end{tikzpicture}
    \caption[\NovelGym{} evaluation protocol and metrics]{Illustration of the \NovelGym{} evaluation protocol and performance over time: the learning agent's success rate (black) as training progresses, with novelty injection (red) dividing the pre- and post-novelty regimes. At initialization the agent's policy is effectively random, so the curve begins at the random-action baseline (blue) and learning lifts it above this chance-level reference. Novelty injection collapses performance back to the chance-level baseline; the post-novelty learning curve then restarts from this baseline point and adaptation gradually recovers performance to $S_{\text{post}}$. The annotated regions correspond to the metrics in \cref{sec:ng_evaluation}: pre-novelty asymptotic performance ($S_{\text{pre}}$), novelty impact ($I_{\text{novelty}}$), time to adapt ($T_{\text{adapt}}$), and post-novelty asymptotic performance ($S_{\text{post}}$).}
...
The benchmark task in \NovelGym{} is a Minecraft-inspired pogostick crafting task. The agent operates in a two-room gridworld environment containing resources (oak trees, diamond ore, platinum blocks), a crafting table, and a trader (see \cref{fig:ng_novelties}\rev{, \cref{fig:ngw2_screenshot}}). The agent must collect resources, craft intermediary items, trade with the trader, and ultimately craft a pogostick. A simplified version of the task was used to make it tractable for reinforcement learning agents while preserving the sequential, multi-step nature of the crafting challenge.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/chapter4/ngw-screenshot.png}
    \caption[\NovelGym{} pogostick crafting task]{\rev{Screenshot of the \NovelGym{} environment running the pogostick crafting task. The agent (blue arrow) navigates a grid containing oak logs (brown), diamond ore (teal), platinum blocks (silver), a crafting table, and a trader (green face). The right panel displays the current episode, step count, agent facing direction, selected action, total cost, and inventory contents.}}
    \label{fig:ngw2_screenshot}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/chapter4/novelties.png}
    \caption[\NovelGym{} novelty scenarios]{Illustration of the pre-novelty environment and three novelty scenarios: fire, fence, and chest. Each transformation modifies specific components of the environment formalism.}
...
\input{chapters/chapter4/ngym_stat_significance}

Additional implementation details, full configuration examples, PDDL specifications, and training hyperparameters are provided in \cref{sec:benchmarks_technical_appendix}.
