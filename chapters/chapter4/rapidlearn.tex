...
%%%%%%%%%%%%%%%%%%%%%%%% ALGO 3: RewardFuncGen %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{algorithm}[t]
\small
\caption{\textit{\textbf{PlannableStateGenerator}} ($\Sigma, \mathcal{P}, o_i) \rightarrow \mathcal{S}_r$}
\label{alg:rewardfuncgen}
\begin{algorithmic}[1] %[1] enables line numbers
% \STATE Input: $\Sigma=\langle\mathcal{E}, \mathcal{F}, \mathcal{S}, \mathcal{O}\rangle$
% \STATE $\Psi = \{o_1^{\psi_1, \psi_2, \ldots}, o_2^{\psi_1, \psi_2, \ldots}, \ldots\}$: preconditions of all the known operators
\STATE $\mathcal{P}=\left[o_{1}, \ldots o_{\mid \mathcal{P}]}\right]$: Plan as an ordered list of operators 
\STATE $\Psi = \{{\psi_{o_1}, \psi_{o_2}, \ldots, \psi_{o_{\mid \mathcal{P}]}} }\}$: Set of preconditions of all the known operators
\STATE $\Omega = \{{\omega_{o_1}, \omega_{o_2}, \ldots, \omega_{o_{\mid \mathcal{P}]}} }\}$: Set of effects of all the known operators


% \STATE  $\Omega = \{o_1^{\omega_1, \omega_2, \ldots}, o_2^{\omega_1, \omega_2, \ldots}, \ldots\}$: effects of all the known operators 
\STATE $\mathcal{S}_r \rightarrow \emptyset$
\FOR{$o_j$ in \textit{reversed}$(\mathcal{P})$}
% \WHILE
\IF{  {$o_j \neq o_i$} $\And$ $\psi_{o_j}  \not\supseteq \omega_{o_i}$}
\STATE$\mathcal{S}_r \cup \psi_{o_j}$
\FOR{$\omega \in \omega_{o_j}$ }
\IF{$\omega \subseteq \mathcal{S}_r$}
\STATE $\mathcal{S}_r \setminus \{\omega\}$
\ENDIF
\ENDFOR
\ENDIF
% \ENDWHILE
\STATE $\mathcal{S}_r \cup \omega_{o_i}$
\ENDFOR
\if
\RETURN $\mathcal{S}_r$
\end{algorithmic}
